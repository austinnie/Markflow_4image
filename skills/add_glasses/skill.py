...
"""
添加/更换眼镜 Skill - 为人物添加或更换眼镜
使用 OpenPose ControlNet 保持姿态，Inpaint 重绘面部区域
"""
...
class AddGlasses:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        logger.info(f"AddGlasses 初始化完成")
    ...
    def _generate_eye_mask(self, image: Image.Image) -> Optional[Image.Image]:
        ...
        try:
            ...
            # 眼部区域（眼镜覆盖区域）
            eye_top = y_min + int(body_h * 0.22)
            eye_bottom = y_min + int(body_h * 0.38)

            x_min, x_max = coords[1].min(), coords[1].max()
            body_w = x_max - x_min
            eye_left = x_min + int(body_w * 0.12)
            eye_right = x_max - int(body_w * 0.12)
            ...
            eye_mask = cv2.GaussianBlur(eye_mask, (11, 11), 0)

            if np.sum(eye_mask > 0) < 100:
                return None
    ...
    def execute(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if eye_mask is None:
                ...
                cx, cy = w // 2, h // 3
                draw.ellipse((cx - w//5, cy - h//8, cx + w//5, cy + h//8), fill=255)
                eye_mask = eye_mask.filter(ImageFilter.GaussianBlur(radius=10))
            ...
            if kwargs.get('output_path'):
                ...
                output_path = str(self.output_dir / f"{Path(image_path).stem}_glasses_{style}_{timestamp}.png")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="添加眼镜工具")

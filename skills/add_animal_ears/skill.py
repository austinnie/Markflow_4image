...
"""
添加兽耳 Skill - 为人物添加动物耳朵（猫耳/狗耳/狐耳等）
使用 OpenPose ControlNet 保持姿态，Inpaint 重绘头部区域
"""
...
class AddAnimalEars:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        logger.info(f"AddAnimalEars 初始化完成")
    ...
    def _generate_head_mask(self, image: Image.Image) -> Optional[Image.Image]:
        """生成头部遮罩（耳朵区域）"""
        ...
        try:
            ...
            # 头部区域（耳朵上方扩展）
            head_top = y_min - int(body_h * 0.15)
            head_bottom = y_min + int(body_h * 0.20)

            x_min, x_max = coords[1].min(), coords[1].max()
            body_w = x_max - x_min
            head_left = x_min - int(body_w * 0.15)
            head_right = x_max + int(body_w * 0.15)

            head_mask = np.zeros_like(combined)
            head_mask[head_top:head_bottom, head_left:head_right] = 255
            head_mask = cv2.bitwise_and(head_mask, combined)
            ...
            logger.info(f"  头部遮罩生成完成")
            return Image.fromarray(head_mask, mode="L")

        except Exception as e:
            logger.warning(f"  头部遮罩生成失败: {e}")
            return None
    ...
    def execute(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            head_mask = self._generate_head_mask(image)
            if head_mask is None:
                ...
                head_mask = head_mask.filter(ImageFilter.GaussianBlur(radius=15))
            ...
            if kwargs.get('output_path'):
                ...
                output_path = str(self.output_dir / f"{Path(image_path).stem}_ears_{animal}_{timestamp}.png")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="添加兽耳工具")

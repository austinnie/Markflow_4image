...
class AddBackgroundObjects:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        logger.info(f"AddBackgroundObjects 初始化完成")
    ...
    def _generate_background_mask(self, image: Image.Image) -> Optional[Image.Image]:
        """生成背景遮罩（反转人物遮罩）"""
        ...
        try:
            ...
            # 反转 -> 背景区域
            bg_mask = 255 - combined
            bg_mask = cv2.GaussianBlur(bg_mask, (11, 11), 0)

            if np.sum(bg_mask > 0) < 100:
                return None

            logger.info(f"  背景遮罩生成完成")
            return Image.fromarray(bg_mask, mode="L")

        except Exception as e:
            logger.warning(f"  背景遮罩生成失败: {e}")
            return None
    ...
    def execute(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            bg_mask = self._generate_background_mask(image)
            if bg_mask is None:
                h, w = image.size[1], image.size[0]
                bg_mask = Image.new("L", (w, h), 0)
                draw = ImageDraw.Draw(bg_mask)
                cx, cy = w // 2, h // 2
                draw.ellipse((cx - w//2.5, cy - h//2.5, cx + w//2.5, cy + h//2.5), fill=255)
                bg_mask = bg_mask.filter(ImageFilter.GaussianBlur(radius=10))
            ...
            if kwargs.get('output_path'):
                ...
                output_path = str(self.output_dir / f"{Path(image_path).stem}_add_{obj}_{timestamp}.png")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="添加背景物体工具")

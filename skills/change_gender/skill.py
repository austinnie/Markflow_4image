...
使用 OpenPose ControlNet 保持姿态，Inpaint 重绘面部 + 发型 + 身体
"""
...
class ChangeGender:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        logger.info(f"ChangeGender 初始化完成")
    ...
    def _generate_upper_body_mask(self, image: Image.Image) -> Optional[Image.Image]:
        """生成上半身遮罩（面部 + 头发 + 上半身）"""
        ...
        try:
            ...
            # 上半身区域：头部到胸部
            top = y_min
            bottom = y_min + int(body_h * 0.55)

            x_min, x_max = coords[1].min(), coords[1].max()
            body_w = x_max - x_min
            left = x_min + int(body_w * 0.05)
            right = x_max - int(body_w * 0.05)

            body_mask = np.zeros_like(combined)
            body_mask[top:bottom, left:right] = combined[top:bottom, left:right]
            ...
            logger.info(f"  上半身遮罩生成完成")
            return Image.fromarray(body_mask, mode="L")

        except Exception as e:
            logger.warning(f"  上半身遮罩生成失败: {e}")
            return None
    ...
    def execute(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            body_mask = self._generate_upper_body_mask(image)
            if body_mask is None:
                ...
                body_mask = body_mask.filter(ImageFilter.GaussianBlur(radius=15))
            ...
            if kwargs.get('output_path'):
                ...
                output_path = str(self.output_dir / f"{Path(image_path).stem}_gender_{direction}_{timestamp}.png")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="性别转换工具")
